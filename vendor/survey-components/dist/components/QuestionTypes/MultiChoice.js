...
const MultiChoice = ({ question, onAnswer, disabled }) => {
    const [selected, setSelected] = useState(new Set());
    const exclusiveValues = useMemo(() => (question.options || [])
        .filter(o => o.exclusive || o.value === 'no-updates')
    ...
    const isOptionDisabled = (optionValue) => {
        ...
        if (hasExclusiveSelected && !isExclusive && !isSelected)
            return true;
        // If any non-exclusive is selected, disable exclusive options (unless it's the selected one)
        if (hasNonExclusiveSelected && isExclusive && !isSelected)
            return true;
        // Enforce maxSelections
        if (question.maxSelections && selected.size >= question.maxSelections && !isSelected)
            return true;
        return false;
    };
    ...
    return (React.createElement(Container, null,
        React.createElement(OptionsContainer, null, question.options?.map((option) => (React.createElement(OptionButton, { key: option.id, onClick: () => handleToggle(option.value), "$isSelected": selected.has(option.value), disabled: isOptionDisabled(option.value) },
            React.createElement(Checkbox, { "$isSelected": selected.has(option.value) }),
            React.createElement(OptionLabel, null, option.label))))),
        React.createElement(SubmitButton, { onClick: handleSubmit, disabled: disabled || selected.size === 0 },
            "Continue (",
            selected.size,
            " selected)")));
};
